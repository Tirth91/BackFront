const labelMap = [
  ...
  const APP_ID = "0f3fde8ae17c4048bcfc8d69286bc851"; // <- Replace with your real App ID
  const CHANNEL = "test";
  const TOKEN = null;
  ...
  function onResults(results) {
    ...
    if (landmarks.reduce((a, b) => a + b, 0) !== 0) {
      ...
      prediction.array().then((probs) => {
        ...
        localPrediction = finalLabel;
        predictionSpan.textContent = finalLabel;
  
        // Send to others
        if (dataStreamId) {
          client.sendStreamMessage(dataStreamId, finalLabel);
        }
      });
  ...
  async function joinCall() {
    ...
    client.on("stream-message", (uid, message) => {
      ...
      if (labelDiv) {
        labelDiv.innerText = `User ${uid}: ${message}`;
      }
    });
  }
